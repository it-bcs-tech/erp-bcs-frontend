...
<svelte:head>
	<title>Document Types | DMS | ERP BCS</title>
</svelte:head>

<div class="px-6 py-8 w-full max-w-5xl mx-auto">
	<header class="mb-8 flex justify-between items-end">
		<div>
			<h1 class="text-3xl font-extrabold text-on-surface tracking-tight">Document Types</h1>
			<p class="text-sm font-medium text-on-surface-variant mt-1">Manage standard document types for DMS.</p>
		</div>
			...
			<tbody class="text-sm font-medium divide-y divide-surface-variant/10">
				{#each data.types as type}
					<tr class="hover:bg-surface-container-low/50 transition-colors">
						<td class="p-4 font-bold text-primary">{type.code}</td>
						<td class="p-4 text-on-surface">{type.name}</td>
						<td class="p-4 text-on-surface-variant">{type.description || '-'}</td>
						<td class="p-4 text-center">
							<span class="px-3 py-1 rounded-full text-xs font-extrabold tracking-wider border {type.is_active ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-700 border-slate-200'}">
	...
	<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
		<div class="bg-surface-container-lowest rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
			<div class="p-6 border-b border-surface-variant/20 flex justify-between items-center">
				<h3 class="text-lg font-bold text-on-surface">Add Document Type</h3>
			...
			<form method="POST" action="?/save" use:enhance={() => {
				...
				<div class="p-6 space-y-4">
					<div>
						...
						<input type="text" name="code" required class="w-full bg-surface-container rounded-xl px-4 py-2.5 text-sm font-medium border-none focus:ring-2 focus:ring-primary outline-none" placeholder="e.g. CONTRACT" />
					</div>
					<div>
						...
						<input type="text" name="name" required class="w-full bg-surface-container rounded-xl px-4 py-2.5 text-sm font-medium border-none focus:ring-2 focus:ring-primary outline-none" placeholder="e.g. Contract Document" />
					</div>
					<div>
						<label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1">Description</label>
						<textarea name="description" rows="2" class="w-full bg-surface-container rounded-xl px-4 py-2.5 text-sm font-medium border-none focus:ring-2 focus:ring-primary outline-none"></textarea>
					</div>
				</div>
