<script lang="ts">
	...
	let filteredLocations = $derived.by(() => {
		if (!searchQuery.trim()) return data.locations || [];
...
<svelte:head>
	<title>Master Lokasi Penyimpanan Berkas Fisik | DMS | ERP BCS</title>
</svelte:head>
	...
	<header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
		<div>
			<div class="flex items-center gap-2.5">
				<span class="material-symbols-outlined text-indigo-600 dark:text-indigo-400 text-2xl">inventory_2</span>
				<h1 class="text-2xl font-black text-on-surface tracking-tight">Lokasi Penyimpanan Berkas Fisik</h1>
			</div>
			<p class="text-on-surface-variant font-medium text-xs sm:text-sm mt-0.5">
				Kelola hierarki lemari, rak, brankas tahan api, dan slot arsip berkas asli perusahaan
			</p>
		</div>
		<button
			type="button"
	...
	<div class="p-4 rounded-2xl bg-surface-container-low border border-slate-200/60 dark:border-slate-800/60 shadow-xs flex items-center justify-between gap-4">
		<div class="relative flex-1 max-w-md">
			...
			<input
				...
				placeholder="Cari kode rak atau nama lemari..."
		...
		<span class="text-xs font-medium text-on-surface-variant">
			Total: <strong class="text-on-surface">{filteredLocations.length}</strong> lokasi
		</span>
	</div>
	...
	<div class="rounded-2xl bg-surface-container-low border border-slate-200/60 dark:border-slate-800/60 overflow-hidden shadow-xs">
		<table class="w-full text-left text-sm">
			<thead class="bg-slate-100/70 dark:bg-slate-800/50 text-xs font-bold text-on-surface-variant uppercase tracking-wider border-b border-slate-200/60 dark:border-slate-800/60">
				<tr>
					<th class="py-3.5 px-5">Kode Lokasi</th>
					<th class="py-3.5 px-5">Nama Lemari / Rak</th>
					<th class="py-3.5 px-5">Deskripsi Kapasitas & Zona</th>
				...
				{#if filteredLocations.length === 0}
					<tr>
						<td colspan="4" class="py-12 text-center text-on-surface-variant">
							<span class="material-symbols-outlined text-4xl text-slate-300 dark:text-slate-600 mb-2">shelves</span>
							<p class="text-xs font-semibold">Tidak ada lokasi penyimpanan yang cocok.</p>
						</td>
					</tr>
				{:else}
					{#each filteredLocations as loc}
						<tr class="hover:bg-surface-container-high/40 transition-colors">
							...
							<td class="py-3.5 px-5 font-bold text-on-surface text-sm">{loc.name}</td>
							<td class="py-3.5 px-5 text-on-surface-variant max-w-md">{loc.description || '-'}</td>
							<td class="py-3.5 px-5 text-center">
								<span class="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-0.5 rounded-full border {loc.is_active ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-slate-100 text-slate-700 border-slate-300'}">
									<span class="w-1.5 h-1.5 rounded-full {loc.is_active ? 'bg-emerald-500' : 'bg-slate-400'}"></span>
									{loc.is_active ? 'Tersedia' : 'Penuh / Non-Aktif'}
								</span>
							</td>
						</tr>
					{/each}
		...
		<div class="bg-surface-container-lowest rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-150">
			<div class="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
				<div class="flex items-center gap-2">
					...
					<h3 class="text-base font-extrabold text-on-surface">Tambah Lokasi Rak / Lemari</h3>
				</div>
				...
				<div class="p-6 space-y-4">
					<div>
						<label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">
							Kode Rak / Lemari <span class="text-rose-500">*</span>
						</label>
						<input
							type="text"
							name="code"
							required
							placeholder="Misal: RAK-A1, BRANKAS-01"
					...
					<div>
						<label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">
							Nama Lokasi Penyimpanan <span class="text-rose-500">*</span>
						</label>
						<input
							type="text"
							name="name"
							required
							placeholder="Misal: Lemari A - Rak 1 (Dokumen Armada)"
