<script lang="ts">
	...
	let tariffs = $derived(data.tariffs || []);
	let metrics = $derived(data.metrics);
	let meta = $derived(data.meta);

	let searchQuery = $state($page.url.searchParams.get('search') || '');
	let zoneFilter = $state($page.url.searchParams.get('zone') || 'All');
	let vehicleFilter = $state($page.url.searchParams.get('vehicle') || 'All');
	...
	function updateQueryParams() {
		...
		if (zoneFilter && zoneFilter !== 'All') url.searchParams.set('zone', zoneFilter);
		else url.searchParams.delete('zone');
		if (vehicleFilter && vehicleFilter !== 'All') url.searchParams.set('vehicle', vehicleFilter);
	...
	function handleFilterChange() { updateQueryParams(); }
...
<svelte:head>
	<title>Tariffs & Pricing | Marketing</title>
</svelte:head>
...
<div class="flex flex-col h-full">
	...
	<header class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
		<div>
			<h1 class="text-3xl font-extrabold text-on-surface tracking-tight mb-2">Tariffs & Pricing</h1>
			<p class="text-on-surface-variant font-medium text-sm">Manage route-based tariffs, zone pricing, and vehicle rate cards</p>
		</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
		<div class="bg-surface-container-lowest p-5 rounded-2xl border border-surface-container shadow-sm">
			<p class="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2">Total Tariffs</p>
			<div class="flex items-end justify-between">
				<h3 class="text-3xl font-black text-on-surface">{metrics.totalTariffs}</h3>
				<span class="material-symbols-outlined text-3xl text-surface-variant">price_change</span>
			</div>
		</div>
		<div class="bg-surface-container-lowest p-5 rounded-2xl border border-emerald-500/20 shadow-sm">
			<p class="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-2">Active Rates</p>
			<div class="flex items-end justify-between">
				<h3 class="text-3xl font-black text-emerald-600">{metrics.activeRates}</h3>
		...
		<div class="bg-surface-container-lowest p-5 rounded-2xl border border-blue-500/20 shadow-sm">
			<p class="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">Zones Covered</p>
			<div class="flex items-end justify-between">
				<h3 class="text-3xl font-black text-blue-600">{metrics.zones}</h3>
				<span class="material-symbols-outlined text-3xl text-blue-500/50">map</span>
			</div>
		</div>
		<div class="bg-surface-container-lowest p-5 rounded-2xl border border-rose-500/20 shadow-sm">
			<p class="text-xs font-bold text-rose-600 uppercase tracking-wider mb-2">Avg. Price / KM</p>
			<div class="flex items-end justify-between">
				<h3 class="text-2xl font-black text-rose-600">{formatCurrency(metrics.avgPricePerKm)}</h3>
				<span class="material-symbols-outlined text-3xl text-rose-500/50">speed</span>
			</div>
		</div>
	</div>
	...
	<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
		<div class="flex gap-3">
			<select bind:value={zoneFilter} onchange={handleFilterChange}
				...
				<option value="Java West">Java West</option>
				<option value="Java Central">Java Central</option>
				<option value="Java East">Java East</option>
				<option value="Banten">Banten</option>
			</select>
			<select bind:value={vehicleFilter} onchange={handleFilterChange}
				class="bg-surface-container-lowest border border-outline-variant/30 text-on-surface rounded-xl py-2 px-4 focus:outline-none focus:ring-2 focus:ring-rose-500/50 text-sm font-medium shadow-sm appearance-none cursor-pointer">
		...
		<div class="relative w-full lg:w-72 flex-shrink-0">
			...
			<input type="text" bind:value={searchQuery} oninput={handleSearchInput}
				placeholder="Search route or tariff ID..." 
			...
			<table class="w-full text-left border-collapse min-w-[1100px]">
				<thead>
					<tr class="border-b border-surface-container sticky top-0 bg-surface-container-lowest z-10">
						...
						<th class="py-5 px-6 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Zone & Vehicle</th>
						<th class="py-5 px-6 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Distance</th>
						<th class="py-5 px-6 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Pricing</th>
						...
						<tr class="group hover:bg-surface-container-low transition-colors">
							<td class="py-4 px-6">
								<div class="flex items-center gap-3">
									...
									<div>
										<p class="text-sm font-bold text-on-surface">{tariff.origin} → {tariff.destination}</p>
										<span class="text-[10px] font-medium text-on-surface-variant/70 uppercase tracking-widest">{tariff.id}</span>
									</div>
								</div>
							</td>
							<td class="py-4 px-6">
								<div class="flex flex-col gap-1.5">
									<span class="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-md bg-surface-container-high text-on-surface-variant w-fit">
										<span class="material-symbols-outlined text-[12px]">map</span>
										{tariff.zone}
									</span>
									<span class="text-[11px] font-medium text-on-surface-variant">{tariff.vehicleType}</span>
								</div>
							</td>
							<td class="py-4 px-6">
								<span class="text-sm font-bold text-on-surface">{tariff.distance} km</span>
							</td>
							<td class="py-4 px-6">
								<div class="flex flex-col gap-1">
									<span class="text-sm font-black text-rose-600 dark:text-rose-400">{formatCurrency(tariff.basePrice)}</span>
									<span class="text-[11px] font-medium text-on-surface-variant">{formatCurrency(tariff.pricePerKm)} / km</span>
								</div>
							</td>
							<td class="py-4 px-6">
							...
							<td class="py-4 px-6 text-right">
								<div class="flex items-center justify-end gap-2">
									<button class="p-2 rounded-lg text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-900/30 transition-colors" title="Edit Tariff">
