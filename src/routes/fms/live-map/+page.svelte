		...
		if (mapReady && displayUnits) {
			...
			if (selectedUnit) {
				...
				if (selectedUnit.originLat && selectedUnit.originLng && selectedUnit.destLat && selectedUnit.destLng) {
					...
					fetch(`http://localhost:8081/api/fms/route?startLat=${selectedUnit.originLat}&startLng=${selectedUnit.originLng}&endLat=${selectedUnit.destLat}&endLng=${selectedUnit.destLng}`)
						.then(r => r.json())
						.then(routeData => {
					...
					fetch(`http://localhost:8081/api/fms/route?startLat=${selectedUnit.lat}&startLng=${selectedUnit.lng}&endLat=${selectedUnit.destLat}&endLng=${selectedUnit.destLng}`)
						.then(r => r.json())
						.then(routeData => {
