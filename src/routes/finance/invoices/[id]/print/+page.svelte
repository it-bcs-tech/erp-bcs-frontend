			...
			<div>
				...
				<div class="grid grid-cols-2 gap-3 mb-3">
					...
					<div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
						...
						<p class="font-black text-xs text-slate-900">{data.invoice.customer_name || '-'}</p>
						{#if data.invoice.customer_code}
							<p class="text-[9.5px] text-slate-500 font-mono">Kode: {data.invoice.customer_code}</p>
						{/if}
					...
					<div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
						...
						<div class="grid grid-cols-2 gap-y-1 gap-x-2 text-[10px]">
							...
							<span class="text-slate-500">No. LHP / Ref:</span>
				...
				<!-- 4. LINE ITEMS TABLE (Compact layout for up to 10+ rows) -->
				<table class="w-full border-collapse border border-slate-300 mb-2.5 text-[10px]">
					<thead>
						<tr class="bg-slate-100 text-slate-900 font-bold border-b border-slate-300">
							...
							<th class="border border-slate-300 py-1.5 px-2 text-left">Deskripsi / Keterangan Layanan</th>
				...
				<div class="grid grid-cols-12 gap-3 mt-2">
					...
					<div class="col-span-7 flex flex-col justify-between">
						...
						{#if data.invoice.notes}
							<div class="mt-1.5 p-2 bg-slate-50 rounded-lg border border-slate-200 text-[9.5px]">
								<p class="text-[8.5px] font-black uppercase tracking-wider text-slate-500 mb-0.5">Catatan / Remarks:</p>
								<p class="text-slate-700 whitespace-pre-wrap leading-tight">{data.invoice.notes}</p>
							</div>
						{/if}
					</div>
					...
					<div class="col-span-5 bg-slate-50 rounded-lg border border-slate-200 p-2 space-y-1 text-[10px]">
						...
						<div class="border-t-2 border-slate-800 pt-1 mt-1 flex justify-between items-baseline">
							...
							<span class="text-sm font-black font-mono text-blue-950">
				...
				<!-- 6. PAYMENT INSTRUCTION & SIGNATURES (Guaranteed on same page) -->
				<div class="signature-block break-inside-avoid grid grid-cols-2 gap-4 mt-3 pt-2 border-t border-slate-200">
					<!-- Bank Account Details -->
					<div>
						<h4 class="text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">
							Instruksi Pembayaran Transfer
						</h4>
						<div class="p-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] space-y-0.5">
							<p class="font-black text-slate-900">{data.invoice.bank_name || 'Bank Mandiri'}</p>
							<p class="font-mono font-bold text-xs tracking-wider text-blue-900">
								{data.invoice.account_number || '-'}
							</p>
							<p class="text-slate-600 text-[9.5px]">a.n <strong class="text-slate-900">{data.invoice.account_name || 'PT BUANA CENTRA SWAKARSA'}</strong></p>
						</div>
						<p class="text-[8.5px] text-slate-500 mt-0.5 italic">
					...
					<div class="text-center flex flex-col justify-between items-center h-20">
						<div>
							<p class="text-[10.5px] font-bold text-slate-700 leading-tight">Hormat Kami,</p>
							<p class="text-[9.5px] text-slate-500">PT. Buana Centra Swakarsa</p>
						</div>
						
						<div>
							<p class="font-bold text-[10.5px] text-slate-900 border-b border-slate-800 inline-block px-4 pb-0.5">
