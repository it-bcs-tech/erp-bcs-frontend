...
<svelte:head>
	<title>Deposit Vendor | Finance ERP</title>
</svelte:head>
	...
	<header class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
		<div>
			<div class="flex items-center gap-2.5">
				...
				<h1 class="text-2xl font-black text-on-surface tracking-tight">Pengeluaran Deposit Vendor</h1>
			</div>
			<p class="text-on-surface-variant font-medium text-xs sm:text-sm mt-0.5 ml-8">
				Pencatatan uang muka pembelian / deposit ke vendor sebelum tagihan resmi diterima
			</p>
		</div>
	</header>
		...
		<div class="space-y-6">
			<div class="p-6 rounded-2xl bg-surface-container-low border border-slate-200/60 dark:border-slate-800/60 shadow-xs space-y-4">
				<h3 class="text-sm font-bold text-on-surface uppercase tracking-wider border-b border-slate-200/60 dark:border-slate-800/60 pb-3 flex items-center gap-2">
					...
					<span>Rincian Deposit ke Vendor</span>
				</h3>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						<label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">
							Sumber Rekening Kas / Bank <span class="text-rose-500">*</span>
						</label>
						<select
							name="accountId"
							required
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						...
						<input
							...
							placeholder="Misal: VDEP-BCA-001"
					...
					<div>
						...
						<input
							...
							name="notes"
							bind:value={notes}
							placeholder="Keterangan deposit vendor..."
			...
			<div class="flex items-center justify-end gap-3 pt-2">
				...
				>
					...
					{:else}
						...
						<span>Simpan Deposit Vendor</span>
					{/if}
				</button>
			</div>
		</div>
